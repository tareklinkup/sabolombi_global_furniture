		...
		methods: {
			...
			getSales() {
				axios.post('/get_sales', this.filter)
					.then(res => {
						// let sales = res.data.sales.filter(sale => sale.SaleMaster_TotalSaleAmount > 0);
						// sales = _.groupBy(sales, 'SalseCustomer_IDNo');
						// sales = _.toArray(sales);
						// sales = sales.map(sale => {
						// 	sale[0].totalAmount = sale.reduce((p, c) => {
						// 		return p + parseFloat(c.SaleMaster_TotalSaleAmount)
						// 	}, 0);
						// 	return sale[0];
						// })
						// this.sales = sales;
						let sales = res.data.sales.filter(sale => sale.SaleMaster_PaidAmount > 0 || sale.SaleMaster_BankPaidAmount > 0);
						sales = _.groupBy(sales, 'SalseCustomer_IDNo');
						sales = _.toArray(sales);
						sales = sales.map(sale => {
							sale[0].totalAmount = sale.reduce((p, c) => {
								return p + parseFloat(+c.SaleMaster_PaidAmount + +c.SaleMaster_BankPaidAmount)
							}, 0);
							return sale[0];
						})
						this.sales = sales;
