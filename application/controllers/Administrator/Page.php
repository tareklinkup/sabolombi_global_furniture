...
class Page extends CI_Controller
{
    ...
    {
        $inptObj = json_decode($this->input->post('data'));

        $companyInfo = $this->db->query("select * from tbl_company")->row();

        if (!empty($_FILES)) {

            if ($companyInfo->Company_Logo_org != '' && file_exists('./uploads/company_profile_org/' . $companyInfo->Company_Logo_org)) {
                unlink('./uploads/company_profile_org/' . $companyInfo->Company_Logo_org);
            }

            if ($companyInfo->Company_Logo_thum != '' && file_exists('./uploads/company_profile_thum/' . $companyInfo->Company_Logo_thum)) {
                unlink('./uploads/company_profile_thum/' . $companyInfo->Company_Logo_thum);
            }
            ...
            $config['image_library'] = 'gd2';
            $config['source_image'] = './uploads/company_profile_org/' . $file_name;
            $config['new_image'] = './uploads/company_profile_thum/';
            $config['maintain_ratio'] = false;
            $config['width']    = 165;
            $config['height']   = 175;
        ...
        $info = (array)$inptObj;
        $info['Company_Logo_org'] = $file_name;
        $info['Company_Logo_thum'] = $file_name;
        $this->db->where('Company_SlNo', $inptObj->Company_SlNo)->update('tbl_company', $info);

        $res = ['statu' => true, 'message' => "Company Updated Successfully"];
    ...
    {
        $res = ['success' => false, 'message' => ''];
        $branchObj = json_decode($this->input->post('data'));

        $nameCount = $this->db->query("select * from tbl_brunch where Brunch_name = ?", $branchObj->name)->num_rows();

        if ($nameCount > 0) {
            $res = ['success' => false, 'message' => $branchObj->name . ' already exists'];
            echo json_encode($res);
            exit;
        }
        ...
        try {
            ...
            $branchInfo       = [];
            $branchInfo['Brunch_name']    = $branchObj->name;
            $branchInfo['Brunch_title']   = $branchObj->title;
            $branchInfo['Brunch_address'] = $branchObj->address;
            $branchInfo['is_production']  = $branchObj->is_production == true ? 'true' : 'false';
            $branchInfo['Brunch_sales']   = '2';
            $branchInfo['add_by']         = $this->session->userdata("FullName");
            $branchInfo['add_time']       = date('Y-m-d H:i:s');
            $branchInfo['status']         = 'a';

            $this->db->insert('tbl_brunch', $branchInfo);
            ...
            if (!empty($_FILES)) {
                ...
                if (!empty($_FILES['Logo'])) {
                    $config['upload_path'] = './uploads/branch_logo_others/';
                    $config['allowed_types'] = 'gif|jpg|png';
                    $this->load->library('upload', $config);
                    ...
                    $upload_data = $this->upload->data(); //Returns array of containing all of the data related to the file you uploaded.
                    $file_name = $upload_data['file_name'];
                    $this->db->set('Logo', $file_name)->where('brunch_id ', $inserId)->update('tbl_brunch');
                }
                ...
                if (!empty($_FILES['QR_Image'])) {
                    ...
                    $config['allowed_types'] = 'gif|jpg|png';
                    $this->load->library('upload', $config);
                    $this->upload->do_upload('QR_Image');

                    $upload_data = $this->upload->data(); //Returns array of containing all of the data related to the file you uploaded.
                    $file_name = $upload_data['file_name'];
                    $this->db->set('QR_Image', $file_name)->where('brunch_id ', $inserId)->update('tbl_brunch');
                }
            }
    ...
    public function updateBranch()
    {
        ...
        $nameCount = $this->db->query("select * from tbl_brunch where Brunch_name = ? and brunch_id != ?", [$branchObj->name, $branchObj->branchId])->num_rows();
        ...
        try {
            ...
            $branchInfo['Brunch_address'] = $branchObj->address;
            $branchInfo['is_production']  = $branchObj->is_production == true ? 'true' : 'false';
            $branchInfo['update_by']      = $this->session->userdata("FullName");

            $this->db->set($branchInfo)->where('brunch_id', $branchObj->branchId)->update('tbl_brunch');
            ...
            if (!empty($_FILES)) {
                ...
                if (!empty($_FILES['Logo'])) {
                    ...
                    $upload_data = $this->upload->data(); //Returns array of containing all of the data related to the file you uploaded.
                    $file_name = $upload_data['file_name'];
                    $this->db->set('Logo', $file_name)->where('brunch_id ', $branchObj->branchId)->update('tbl_brunch');
                }
                ...
                if (!empty($_FILES['QR_Image'])) {
                    ...
                    $upload_data = $this->upload->data(); //Returns array of containing all of the data related to the file you uploaded.
                    $file_name = $upload_data['file_name'];
                    $this->db->set('QR_Image', $file_name)->where('brunch_id ', $branchObj->branchId)->update('tbl_brunch');
                }
            }
    ...
    {
        $query = $this->db->query("SELECT * FROM tbl_color where status='a' order by color_name asc")->result();
