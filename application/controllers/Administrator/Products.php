...
class Products extends CI_Controller
{
    ...
    public function addProduct()
    {
        ...
        try {
            ...
            if ($productNameCount > 0) {
                ...
                $res = ['success' => false, 'message' => 'The Product already exists with Same color and size', 'productId' => $Product_Code];
    ...
    public function updateProduct()
    {
        ...
        try {
            ...
            $productNameCount = $this->db->query("select * from tbl_product where Product_Name = ? and Product_SlNo != ? and color = ? and size = ?", [$productObj->Product_Name, $productObj->Product_SlNo, $productObj->color, $productObj->size])->num_rows();
            if ($productNameCount > 0) {
                $res = ['success' => false, 'message' => 'The Product already exists with Same color and size', 'productId' => $productObj->Product_Code];
                echo json_encode($res);
                exit;
            }
            ...
            if (!empty($_FILES)) {
                ...
                if ($productImage != '' && file_exists('./uploads/products/' . $productImage)) {
                    unlink('./uploads/products/' . $productImage);
                }
    ...
    public function getProducts()
    {
        ...
        $products = $this->db->query("
            select
                p.*,
                concat(p.Product_Name, ' - ', ifnull(c.color_name, ''), ' - ', ifnull(p.size, ''), ' - ', p.Product_Code) as display_text,
    ...
    public function getTotalStock()
    {
        ...
        $stock = $this->db->query("
            ...
            left join tbl_unit u on u.Unit_SlNo = p.Unit_ID
            where Product_branchid = '$branchId' and p.status = 'a' and p.is_service = 'false' $clauses
        ")->result();
